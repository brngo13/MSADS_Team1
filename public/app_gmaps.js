...
function initMap() {
    ...
    const mapDefaults = config.mapDefaults || { center: { lat: 39.8283, lng: -98.5795 }, zoom: 5 };
...
/**
 * Load facility data from CSV or API
 * Data source configured in config.json
 */
async function loadFacilityData() {
    try {
        ...
        const dataUrl = dataSource.type === 'api' ? dataSource.apiEndpoint : dataSource.path;
        ...
        Papa.parse(csvText, {
            ...
            complete: (results) => {
                ...
                // Apply default filters (1000 tons minimum)
                applyFilters();
...
/**
 * Get colors for risk level (fill and stroke)
 */
function getRiskColors(riskLevel) {
    const colorSets = {
        'High': {
            fill: '#ef4444',    // Red
            stroke: '#991b1b'   // Darker red
        },
        'Medium': {
            fill: '#f59e0b',    // Amber
            stroke: '#d97706'   // Darker amber/orange
        },
        'Low': {
            fill: '#22c55e',    // Green
            stroke: '#15803d'   // Darker green
        },
        'Unknown': {
            fill: '#808080',    // Gray
            stroke: '#404040'   // Darker gray
        }
    };
    return colorSets[riskLevel] || colorSets['Unknown'];
}

/**
 * Get fill color only (for backward compatibility)
 */
function getRiskColor(riskLevel) {
    return getRiskColors(riskLevel).fill;
}
...
function toggleLegend(type, rankType = 'national') {
    ...
    if (type === 'adi') {
        facilityLegend.style.display = 'none';
        ...
        facilityLegend.style.display = 'block';
...
/**
 * Apply risk level and emissions filters
 */
function applyFilters() {
    const riskFilter = document.getElementById('risk-filter').value;
    const emissionsSlider = document.getElementById('emissions-slider');

    // Convert log scale slider (0-5) to actual emissions value (1-100000)
    const minEmissions = Math.pow(10, parseFloat(emissionsSlider.value));

    // Apply both filters
    filteredData = facilityData.filter(f => {
        const totalEmissions = parseFloat(f['total_emissions']) || 0;
        const riskLevel = f['risk_level'];

        // Check risk filter
        const passesRiskFilter = (riskFilter === '' || riskLevel === riskFilter);

        // Check emissions filter
        const passesEmissionsFilter = totalEmissions >= minEmissions;

        return passesRiskFilter && passesEmissionsFilter;
    });
...
function setupEventListeners() {
    ...
    // Emissions slider - update display value
    const emissionsSlider = document.getElementById('emissions-slider');
    ...
    emissionsSlider.addEventListener('input', (e) => {
        ...
        if (value >= 1000) {
            emissionsValue.textContent = (value / 1000).toFixed(1).replace(/\.0$/, '') + 'k';
        } else {
            emissionsValue.textContent = value.toFixed(0);
        }
    });
    ...
    // Show/hide ADI rank type selector based on indicator selection
    document.getElementById('social-indicator-selector').addEventListener('change', (e) => {
        const rankTypeSection = document.getElementById('adi-rank-type-section');
        if (e.target.value === 'adi') {
            rankTypeSection.style.display = 'block';
        } else {
            rankTypeSection.style.display = 'none';
        }
    });

    // Census boundary radio buttons
    document.querySelectorAll('input[name="census-boundary"]').forEach(radio => {
        radio.addEventListener('change', (e) => {
            handleCensusBoundaryChange(e.target.value);
        });
    });
...
function showToast(message, type = 'info') {
    // Simple console log for now - can be enhanced with UI toast later
